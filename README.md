...
The token generated by `LoginManager.login()` is a JWT for HTTP web servers. It is supposed to send to the client (browser) as a proof of authentication. Every once the client sent it back to your server, `AccessManager.verify()` should be applied to verify the token:
